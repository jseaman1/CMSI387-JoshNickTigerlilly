...
//      printf("\nLine 131\n");
  ...
  else if (! (no_create || no_dereference))
    {
      ...
      int nfd = fd_reopen( 0, file, O_WRONLY, MODE_RW_UGO );
//      printf("\nfd: %d\n\n", nfd);
      if ( nfd == STDIN_FILENO )
          printf("(︶︹︶) this file already exists, sorry bud (︶︹︶)\n");
      else
          printf("=^_^= You made a new file: %s\n", file);
...
//      printf("\nLine 140\n");
      ...
      if (fd == -1 && errno != EISDIR && errno != EINVAL && errno != EPERM)
        {
	  printf("༼ ͡☉ ͜ʖ ͡☉ ༽  Does this look like a file to you?!\n");
          open_errno = errno;
        }
    }
...
"), stdout);
  ...
  -b			 beej addition part 1 :)\n\
  -j			 jeeb addition part 2 :)\n\
"), stdout);
      fputs (_("\
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "acd:fbjhmr:t:", longopts, NULL)) != -1)
    {
      switch (c)
        {
        case 'a':
  ...
  if (beej && jeeb)
    {
      printf("\n\n Princess Jelly says 'ur driving me nuts!'\n----------------------------------------------\n");
      puts("         _.-\"\"\"-,");
      puts("       .'  ..::. `\\");
      puts("      /  .::' `'` /");
      puts("     / .::' .--.=;");
    ...
    {
      printf("\n\n Sir Peanut Butter says 'gimme ur peanuts!'\n----------------------------------------------\n");
      puts("   )\" .");
      puts("  /    \\     (\\-./");
      puts(" /     |    _/ o. \\");
      puts("|      | .-\"      y)-");
  ...
  if (optind == argc)
    {
      error (0, 0, _("¯\\_( ͡°ᗝ ͡°)_/¯ You reeeally forgot the file??? *tsk* *tsk*"));
      usage (EXIT_FAILURE);
    }
